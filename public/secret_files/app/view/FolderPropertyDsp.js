...
Ext.define('TeamDomain.view.FolderPropertyDsp', {
    extend: 'Ext.window.Window',
    alias: 'widget.folderpropertydsp',
    height: 600,
    hidden: false,
    id: 'folderpropertydsp',
    itemId: 'folderpropertydsp',
    width: 500,
    modal: true,
    layout: {
        align: 'stretch',
        ...
        Ext.applyIf(me, {
            items: [
                ...
                },
                {
                            ...
                            items: [
                                {
                                    xtype: 'form',
                                    frame: true,
                                    id: 'showFolderPropertyDsp',
                                    itemId: 'showFolderPropertyDsp',
                                    autoScroll: true,
                                    bodyPadding: 5,
                                    title: '基本',
                                    items: [
                    ...
                    listeners: {
                        afterrender: {
                            fn: me.onFolderPropertyDispAfterRender,
                            scope: me
                        }
                    }
                }
    ...
    onFolderPropertyDispAfterRender: function (component, eOpts) {
        var grid;
        var model;
        var record;

        if (TargetComp === 'listGridPanelA') {
            grid = Ext.getCmp('listGridPanelA');
            model = grid.getSelectionModel();
            record = model.getSelection()[0];
        } else {
            grid = Ext.getCmp('folderPanelA');
            model = grid.getSelectionModel();
            record = model.getSelection()[0];
            Ext.getCmp('showFolderPropertyDsp').getForm().setValues({
                change_file_name: record.data.text,
                modifier: record.data.updater
            });
            
            var send_data = Ext.apply({session_id: this_session_id}, {cont_location: record.data.cont_location});
            send_data = Ext.apply(send_data, {hash_key: record.data.hash_key});
            send_data = Ext.apply({request_type: "get_folder_property"}, send_data);

            Ext.Ajax.request({
                url: 'tdx/updatedata.tdx',
                jsonData: send_data,
                success: handleSuccess4,
                failure: handleFailure4
            });
        }
        
        function handleSuccess4(response) {
            obj = Ext.decode(response.responseText);
            var request_success = obj.success;
            var display_data=obj.display_data;
            if (request_success === false) {
                var request_errors = obj.errors;
                Ext.Msg.show({
                    title: 'プロパティ取得失敗',
                ...
                Ext.getCmp('showFolderPropertyDsp').getForm().setValues({
                    change_file_description:display_data.node_name
                });
                Ext.getCmp('showFolderExtensionDsp').getForm().setValues({
                    view_file_name:display_data.node_name,
                    view_file_description:display_data.node_description,
                    change_duration:display_data.memo1,
                    change_producer:display_data.memo2,
                    change_produced_date:display_data.memo3,
                    change_location:display_data.memo4,
                    change_cast:display_data.memo5
                });
                Ext.getCmp('showFolderDetailsDsp').getForm().setValues({
                    view_file_name2:display_data.node_name,
                    view_file_description2:display_data.node_description,
                    change_details:display_data.details
                });
            }
        }

        function handleFailure4(response) {
            Busy = false;
            Ext.Msg.show({
                title: 'プロパティ取得失敗',
                msg: 'サーバとの通信に失敗しました',
        ...
        try {
            Ext.getCmp('showFolderThumbnailDsp').update(record.data);
            Ext.getCmp('showFolderPropertyDsp').loadRecord(record);
            Ext.getCmp('showFolderExtensionDsp').loadRecord(record);
            Ext.getCmp('showFolderDetailsDsp').loadRecord(record);
