...
class FolderDatum < ActiveRecord::Base
  ...
  def self.get_folder_display_data sid, cont_location
    ...
    when 'folder_a', 'folder_at', 'folder_atfi', 'dlfolders'
      ...
      if td.blank? # => if it isn't
        ...
        target_domain = DatabaseUtility::SessionUtility.get_default_domain(sid)
        if target_domain.blank?
          return tree_nodes
        end
      else
        # => or get domain hash key
        target_domain = td[:spin_domain_hash_key]
      end
      # => get spin_domains record from domain hash key
      current_domain_node = SpinDomain.find_by_hash_key target_domain
    ...
    when 'folder_b'
      ...
      #      target_domain = td[:spin_domain_hash_key]
      #      target_domain = tses[:selected_domain_b]
      if target_domain.blank?
        target_domain = DatabaseUtility::SessionUtility.get_default_domain(sid)
        target_domain_root_folder = SpinDomain.get_domain_root_node_key(target_domain)
      else
        #        tdwrk = DomainDatum.find_by_session_id_and_cont_location_and_hash_key sid, 'folder_b', target_domain
        #        if tdwrk == nil
        sd = SpinDomain.find_by_hash_key target_domain
        if sd.blank?
          return tree_nodes
        end
    ...
    begin
      td = SpinDomain.find_by_hash_key target_domain
      if td.blank?
        return tree_nodes
      end
    ...
    self.transaction do
      if cont_location =~ /folder_[ab]/
        begin
          current_partial_root = self.find_by_session_id_and_domain_hash_key_and_cont_location_and_is_partial_root(sid, target_domain, cont_location, true)
    ...
    # current_parent = nil
    current_parent = self.find_by_session_id_and_domain_hash_key_and_spin_node_hashkey_and_cont_location(sid, target_domain, root_node_key, cont_location)
    # current_parent = self.find_by_session_id_and_domain_hash_key_and_cont_location_and_selected(sid, target_domain, cont_location, true)

    # => make tree data    
    # tree_nodes = make_folder_tree sid, cont_location, nil, root_node_key, target_domain, (uid == 0 ? true : false)
    # if current_parent.present?
    #   tree_nodes.push current_parent
    # end
    tree_nodes += make_folder_tree sid, cont_location, current_parent, target_domain, (uid == 0 ? true : false)
  ...
  def self.make_folder_tree(sid, cont_location, tree_root_folder, target_domain, root_user_access = false)
    ...
    if tree_root_folder.present?
      ...
      # children_recs = self.select("id,spin_node_hashkey").where(["parent_hash_key = ? AND session_id = ? AND cont_location = ? AND domain_hash_key = ? AND vpath <> ?", tree_root_folder[:spin_node_hashkey], sid, cont_location, target_domain, '/']).order("folder_name ASC")
    else
      return tree_nodes
    end
    ...
    children_nodes.each {|cnid|
      ...
      hcurrent['leaf'] = false
      hcurrent['children'] = make_folder_tree(sid, cont_location, cn, target_domain, root_user_access)
      ...
      self.transaction do
        ...
        begin
          ...
          if fd.present?
            if dirty_flag
            ...
            elsif fd[:is_dirty] or !fd[:expanded] or fd[:is_new]
              ...
              self.set_partial_root(sid, location, hkey, domain_key)
              self.fill_folders(sid, location, domain_key, hkey)
              self.select_folder(sid, hkey, location, domain_key)
              # clear and set is_partial_root flag
              #              self.reset_partial_root(sid, location, domain_key)
            elsif fd[:expanded] or fd[:expand_count] > 0
            ...
            if dirty_flag # domain is dirty
              ...
              rethash = {:success => true, :status => STAT_DATA_NOT_LOADED_YET, :isDirty => false, :result => true}
              self.set_partial_root(sid, location, pfd[:spin_node_hashkey], domain_key)
              self.load_folder_recs(sid, hkey, domain_key, fd[:parent_hash_key], location, DEPTH_TO_TRAVERSE, SessionManager.get_last_session(sid))
      ...
      self.transaction do
        ...
        begin
          ...
          if frec.present? # => Use it.
            ...
            FolderDatum.where(session_id: sid, cont_location: cont_location, spin_node_hashkey: folder_key, domain_hash_key: domain_key).update_all(is_dirty: false, leaf: false)
  ...
  def self.set_folder_dirty sid, node_key
    ...
    nfolders = 0
      ...
      self.transaction do
        ...
        begin
          nfolders = FolderDatum.find_by(session_id: sid, spin_node_hashkey: node_key, is_dirty: false).update(is_dirty: true, is_new: false, is_dirty_list: true, expand_counter: 0)
